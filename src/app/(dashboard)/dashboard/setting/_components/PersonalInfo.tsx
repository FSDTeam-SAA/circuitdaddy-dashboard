...
// ✅ FIXED: joiningDate must be Date, not string
const formSchema = z.object({
    firstName: z.string().min(1),
    lastName: z.string().min(1),
    emailAddress: z.string().email(),
    phoneNumber: z.string().min(1),
    address: z.string().min(1),
    joiningDate: z.date().optional(), // <-- FIXED
    lastoginTime: z.string().min(1),
});
...
export default function PersonalInfo() {
    ...
    // ✅ FIXED: API date converted into a proper Date object
    useEffect(() => {
        if (profileData) {
            form.reset({
                firstName: profileData.firstName ?? "",
                ...
                address: profileData.location ?? "",
                emailAddress: profileData.email ?? "",
                joiningDate: profileData.createdAt ? new Date(profileData.createdAt) : undefined, // FIXED
                lastoginTime: profileData.lastLogin
                    ? new Date(profileData.lastLogin).toLocaleString("en-GB")
                    : "",
            });
        }
                ...
                <Form {...form}>
                    ...
                    >
                        ...
                        <div className="flex justify-end">
                            <Button type="submit" className="bg-[#147575] text-white">
                                Submit{" "}
